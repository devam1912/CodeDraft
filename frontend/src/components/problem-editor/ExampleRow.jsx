...
function ExampleRow({ index, example, onChange, onRemove, canRemove }) {
  ...
  return (
    <div className="group relative bg-bg-surface border border-border-default rounded-2xl overflow-hidden transition-all duration-300 hover:border-[#2a2a4e] hover:shadow-[0_0_24px_rgba(126,93,189,0.06)] animate-fade-in">
      {/* Accent top-bar */}
      <div className="h-[2px] bg-gradient-to-r from-secondary via-primary to-secondary opacity-60" />

      <div className="p-5 flex flex-col gap-4">
        {/* Header */}
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <span className="w-7 h-7 flex items-center justify-center rounded-lg bg-secondary/10 text-secondary text-[11px] font-bold font-mono border border-secondary/20">
              {index + 1}
            </span>
            <span className="text-[11px] font-bold text-secondary tracking-[0.08em] uppercase font-sans">
              Example
            </span>
          </div>
          {canRemove && (
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[11px] font-semibold uppercase tracking-[0.1em] text-text-muted">
            Explanation
            <span className="text-text-muted/50 normal-case tracking-normal ml-1.5 text-[10px]">optional</span>
          </label>
          <input
            type="text"
            ...
            className="w-full bg-bg-primary/80 border border-border-default rounded-xl px-3.5 py-2.5 text-sm text-text-primary placeholder-text-muted/60 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-bg-primary transition-all duration-200"
          />
        </div>
      </div>
    </div>
